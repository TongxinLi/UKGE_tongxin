...
class Tester(object):
    ...
    def load_test_data(self, this_data, filename, splitter='\t', line_end='\n'):
    ...
    def classify_triples(self, confT):
    # def classify_triples(self, confT, plausTs):
        ...
        for pthres in np.arange(0, 1, 0.05):
    ...
    def decision_tree_classify(self, confT, train_data):
        ...
        precision = sklearn.metrics.precision_score(test_Y_truth, test_Y_pred)  # 精确度

        recall = sklearn.metrics.recall_score(test_Y_truth, test_Y_pred)  # 召回率

        F1 = sklearn.metrics.f1_score(test_Y_truth, test_Y_pred)  # F1得分
...
class UKGE_rect_Tester(Tester):
    ...
    def get_r2_mse_mae_neg(self, neg_per_positive, verbose=False, save_dir='', epoch=0):
        test_triples = self.test_triples
        N = test_triples.shape[0]
        ...
        r2 = sklearn.metrics.r2_score(w_batch, scores)
        ...
        return r2, mse, mae, mse_neg, mae_neg
