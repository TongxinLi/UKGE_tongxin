...
from UKGE_master_yuan.src import param
...
import time
from UKGE_master_yuan.src.data import BatchLoader

from UKGE_master_yuan.src.utils import vec_length
from UKGE_master_yuan.src.list import ModelList
from UKGE_master_yuan.src.models import UKGE_logi_TF, UKGE_rect_TF
from UKGE_master_yuan.src.testers import UKGE_logi_Tester, UKGE_rect_Tester
...
class Trainer(object):
    ...
    def train(self, epochs=20, save_every_epoch=10, lr=0.001, data_dir=""):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % save_every_epoch == 0:
                ...
                val_r2, val_loss_mse, val_loss_mae, val_loss_mse_neg, val_loss_mae_neg, mean_ndcg, mean_exp_ndcg, text_precision1, text_recall1, text_F11, text_accu1 = self.get_val_loss(
                    epoch,
                    sess)  # loss for testing triples and negative samples
                val_losses.append(
                    [epoch, val_r2, val_loss_mse, val_loss_mae, val_loss_mse_neg, val_loss_mae_neg, mean_ndcg, mean_exp_ndcg, text_precision1,
                     text_recall1, text_F11, text_accu1])
                ...
                self.save_loss(val_losses, self.val_loss_path,
                               columns=['val_epoch', 'r2', 'mse', 'mae', 'mse_neg', 'mae_neg', 'ndcg(linear)', 'ndcg(exp)', 'precision',
                                        'recall', 'F1', 'accu'])
    ...
    def get_val_loss(self, epoch, sess):
        ...
        file_test = join(param.data_dir(), 'test_with_neg.tsv')
        test_triples = self.validator.load_test_data(self.this_data, file_test)
        self.validator.build_by_var(test_triples, self.tf_parts, self.this_data, sess=sess)

        if not hasattr(self.validator, 'hr_map'):
            self.validator.load_hr_map(param.data_dir(), 'test_with_neg.tsv', ['train.tsv', 'val.tsv',
                                                                          'test_with_neg.tsv'])
        ...
        r2, mse, mae, mse_neg, mae_neg = self.validator.get_r2_mse_mae_neg(neg_per_positive=self.neg_per_positive, save_dir=self.save_dir, epoch=epoch)
        # mae = self.validator.get_mae(save_dir=self.save_dir, epoch=epoch)
        # mse_neg = self.validator.get_mse_neg(self.neg_per_positive)
        train_data = pd.read_csv(join(param.data_dir(), 'train.tsv'), sep='\t', header=None,
                                 names=['v1', 'relation', 'v2', 'w'])
        precision1, recall1, F11, accu1 = self.validator.decision_tree_classify(0.7, train_data)
        return r2, mse, mae, mse_neg, mae_neg, mean_ndcg, mean_exp_ndcg, precision1, recall1, F11, accu1
